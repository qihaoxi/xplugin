...
### 4.7 ABI 兼容性纪律(三型公共结构体)

> 教训来源:raw-spofer-pel 的公共头里,句柄类用不透明指针(✔),但
> **info/snapshot/config/stats 类结构体直接暴露字段**(如 `pel_db_stats_snapshot_s`、
> `pel_debug_fibre_info_s`、`pel_fr_config_s`)——它们是库拥有、交给调用方读的数据,
> 一加字段就破坏按值拷贝与布局兼容。xplugin 以此为戒。
>
> ABI 兼容的本质:**消费方编译出的结构体布局 == 库编译出的布局,且符号集是超集**。
> 两个坐标轴决定公共结构体的形态:①谁构造、谁拥有;②调用方是否需要在自有存储上
> 分配(栈上)。三型一型一个制度,不混用。

**类型 A:库拥有、返回给调用方读的数据(provenance/stats/info/snapshot)→ opaque + 访问器**

- 公共头只给 `typedef struct xpl_xxx xpl_xxx_t;`(不透明)+ `xpl_xxx()` 获取 + 逐字段访问器函数。
- 内部布局完全私有:加字段、删字段、重排都不动 ABI;新增暴露字段 = 新增一个访问器函数(尾部追加符号,兼容)。
- 调用方无法按值拷贝、无法取成员地址去依赖布局——这是"字段增长"唯一安全的形态。
- 现行实例:`xpl_build_info_t`(§5.1 之后)。

**类型 B:调用方构造、传给库的输入结构体/vtable(allocator/config/服务契约)→ 公开布局 + version + 尾部追加**

- 调用方必须看得见才能构造,shadow 不适用。
- 制度:首字段 `version`(`uint32_t`)/ 新增只允许尾部追加 / 禁止重排与删除 / 禁止原地改旧字段。
- 库只读调用方已知大小的前缀(`sizeof` 检查),不假设尾部存在;跨进程(remote 代理)服务接口只传可序列化 blob(§8.3)。
- 函数指针 vtable 的"增长单位"是尾部追加槽位,新槽 NULL = 能力不可用(§4 缺省语义)。
- **类型 B 的尾部追加是安全的**(调用方 `sizeof` 变大、库只读已知前缀,不写回)——"直接追加破坏 ABI"只对库拥有且暴露的类型(A/C)成立,勿误伤本型。
- 现行实例:`xpl_allocator`;M1b 起所有服务 vtable。

**类型 C:库拥有、但调用方必须在自有存储上分配(栈上)→ 三种子方案**

场景:无堆分配热路径 / 宿主想自己管内存时,把库对象塞进自己的栈缓冲。此时布局对调用方可见,必须**第一版就定死,禁止后期追加普通成员**。

子方案(优先级从高到低):

1. **sizeof 查询 + init(`size_t f_needed(void); void f_init(void* storage);`)**
   - 100% ABI、可栈分配、**不暴露布局**:调用方只拿不透明指针 + 一段缓冲,布局仍私有。
   - 无浅拷贝问题(没有可见字段)。被忽略但通常是类型 C 最优解。
2. **shadow 指针扩展(公开小结构体尾部预留 `void* shadow`,v1 就必须在)**
   - 规则:原有成员顺序/类型/数量**永不改动**;只在尾部开 `void* shadow`(v1 预留,后期补直接崩——旧程序连 `foo->shadow` 都读不到,读到的是自己栈上相邻内存);所有新字段进堆上的 shadow 子结构体;访问 shadow 成员必须 `if (foo->shadow)` 判空。
   - 代价:①深层冻结首部布局(永久收缩性契约);②**浅拷贝双重 free**——`Foo b = *a;` 只复制指针,须提供 `foo_clone()` 并在文档禁止外部 memcpy/结构体赋值;③不能整块 memdump 序列化(shadow 在堆、不在原结构体块内)。
3. **预留 padding 数组(`uint64_t reserved[N]`)**
   - 全栈、无堆开销;但容量有限、有对齐风险;仅当扩展量可预知且很小。

> **判定表**

| 谁构造 | 谁拥有 | 调用方分配点 | 形态 | 制度 |
|--------|--------|------------|------|------|
| 库 | 库(交给调用方读) | 不需要 | 不透明 + 访问器 | 类型 A:shadow,布局自由 |
| 库 | 库 | **需要(栈上)** | opaque + needed/init,或 shadow 指针,或 padding | 类型 C:布局第一版定死,禁止后加普通成员 |
| 调用方 | 调用方(交给库读) | 无关 | 公开 struct/vtable | 类型 B:version + 尾部追加 |
| 库 | 调用方(回调注入) | 无关 | 函数指针 | 类型 B:尾部追加槽位,NULL=不可用 |

**硬性约束**:类型 A 绝不公开字段;类型 B 绝不 shadow;类型 C 一旦公开,首部布局永久冻结——三个方向的新公共结构体立项时先按此表定型,再写代码。**服务 vtable 一律类型 B**;**影子指针只在特殊需求(栈分配 + 库另有私有状态)才用,默认优先 opaque 与 sizeof-query**。
