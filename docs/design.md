...
> 一加字段就破坏按值拷贝与布局兼容。xplugin 以此为戒,按"谁构造、谁拥有"把公共结构体
> 分两型,一型一个制度,不混用。

**类型 A:库拥有、返回给调用方读的数据结构(provenance/stats/info/snapshot)→ struct shadow**
...
**类型 B:调用方构造、传给库的输入结构体/vtable(allocator/config/服务契约)→ 保持公开布局**
...
| 谁构造 | 谁拥有 | 形态 | 制度 |
|--------|--------|------|------|
| 库 | 库(交给调用方读) | 不透明 + 访问器 | 类型 A:shadow,布局自由 |
| 调用方 | 调用方(交给库读) | 公开 struct/vtable | 类型 B:version + 尾部追加 |
| 库 | 调用方(回调注入) | 函数指针 | 类型 B:尾部追加槽位,NULL=不可用 |

硬性约束:**类型 A 绝不公开字段;类型 B 绝不 shadow**;新公共结构体立项时先按此表定型,再写代码。
